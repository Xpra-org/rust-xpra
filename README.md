...
[`cargo cyclonedx`](https://github.com/CycloneDX/cyclonedx-rust-cargo) from the same build's `Cargo.lock`: the
...
[`cargo auditable`](https://github.com/rust-secure-code/cargo-auditable), so they embed that list too, readable by
